...
def dealer_play(deck: list[dict], dealer: dict) -> bool:
    while True:
        if calculate_hand_rank(dealer['hand']) > 17:
            return True
        add_card(deck,dealer)
        if calculate_hand_rank(dealer['hand'])>21:
...
def run_full_game(deck: list[dict], player: dict, dealer: dict) -> None:
    ...
    while True:
        print(calculate_hand_rank(player['hand']))
        player_enswer = ask_player_action()
        if player_enswer == 'H':
            add_card(deck,player)
            if calculate_hand_rank(player['hand'])>21:
                print (f'!!! you lost with sum of {calculate_hand_rank(player['hand'])} !!!')
                return None
        else: break
        
    
    res = dealer_play(deck,dealer)
    if not res:
        print(f"!!!!  player won with sum of {player_sum} !!!!")
        return None
